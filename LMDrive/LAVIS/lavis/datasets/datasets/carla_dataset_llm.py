...
instruction_json = os.path.join(curr_dir, 'instruction_dict.json')
...
def lidar_to_raw_features(lidar):
    def preprocess(lidar_xyzr, lidar_painted=None):
        ...
        idx = (lidar_xyzr[:, 0] > -1.2) & (lidar_xyzr[:, 0] < 1.2) & (lidar_xyzr[:, 1] > -1.2) & (
                    lidar_xyzr[:, 1] < 1.2)
...
class CarlaVoiceDataset(BaseIODataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.enable_start_frame_augment and len(info[
                                                       'instruction_args']) == 0:  # if instruction_args has no values, it means the instruction doesn't include distance
            if 'Other' not in info['instruction'] or info['instruction'] != 'Follow-01':
                augment_range = min(16, max(0, self.token_max_length * self.sample_interval - (
                            info['end_frame'] - info['start_frame'])))
                start_frame_id = max(0, start_frame_id - random.randint(0, augment_range))
        end_frame_id = min(info['end_frame'] + 1,
                           start_frame_id + self.token_max_length * sample_interval - random.randint(0,
                                                                                                     sample_interval - 1))
        ...
        if self.enable_notice:
            ...
            if route_path in self.notice_data:
                ...
                notice_list = [x for x in notice_list if
                               x['frame_id'] > start_frame_id and x['frame_id'] < end_frame_id - 16]
