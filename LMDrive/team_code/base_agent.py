...
from LMDrive.team_code.planner import RoutePlanner
...
class BaseAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        self.save_skip_frames = self.config.get("save_skip_frames", 2)
        print(self.config)
        ...
        if SAVE_PATH is not None:
            ...
            (self.save_path / "lidar_odd").mkdir(parents=True, exist_ok=True)
    ...
    def save_odd_lidar(
        ...
        np.save(
            self.save_path / "lidar_odd" / ("%04d.npy" % frame),
            tick_data["lidar"].astype(np.float16),
            allow_pickle=True,
        )
